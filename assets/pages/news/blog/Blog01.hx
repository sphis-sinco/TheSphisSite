	...
	override function onCreate(event:CreateEvent)
	{
		...
		BlankPage.instance.pageContent = [
			...
			new PageEvent(new PageEventID(PageEventID.text, {
				...
				text_size: 24,
				text_color: (Preferences.darkMode) ? FlxColor.WHITE : FlxColor.BLACK,
				text_content: "Hehe, another blog technically, but this one I maintain so I'm chill with it :)\n\nAnyway no Idea what the fuck I'm gonna do with this...\n\nI just have wanted to make a site using script files and well... I did it :D"
			}), 'msg'),
		];
	}
