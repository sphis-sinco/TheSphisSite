	...
	private static function registerScriptClass(c:PolymodClassDeclEx)
	{
		...
		if (_scriptClassDescriptors.exists(name))
		{
			Polymod.error(SCRIPT_CLASS_ALREADY_REGISTERED,
				'A scripted class with the fully qualified name "$name" has already been defined. Please change the class name or the package name to ensure a unique name.');
			return;
		}
		else
		{
		...
		function getIdent(e:Expr):Null<String>
		{
				...
				if (getClassDecl() != null)
				{
					...
					if (importedClass != null)
					{
						if (importedClass.cls != null)
							return importedClass.cls;
						if (importedClass.enm != null)
							return importedClass.enm;
			...
			private function buildScriptClassStaticFunction(clsName:String, fieldName:String, fn:FunctionDecl):Dynamic
			{
				...
				switch (argCount)
				{
					...
					case 1:
						return function(a:Dynamic):Dynamic
						{
					...
					case 3:
						return function(a:Dynamic, b:Dynamic, c:Dynamic):Dynamic
						{
							return callScriptClassStaticFunction(clsName, fieldName, [a, b, c]);
						}

					case 4:
						return function(a:Dynamic, b:Dynamic, c:Dynamic, d:Dynamic):Dynamic
						{
					...
					#if neko
					case _:
						@:privateAccess error(ECustom("only 4 params allowed in script class functions (.bind limitation)"));
					#else
					case 5:
						return function(a:Dynamic, b:Dynamic, c:Dynamic, d:Dynamic, e:Dynamic):Dynamic
						{
							return callScriptClassStaticFunction(clsName, fieldName, [a, b, c, d, e]);
						}

					case 6:
						return function(a:Dynamic, b:Dynamic, c:Dynamic, d:Dynamic, e:Dynamic, f:Dynamic):Dynamic
						{
							return callScriptClassStaticFunction(clsName, fieldName, [a, b, c, d, e, f]);
						}

					case 7:
						return function(a:Dynamic, b:Dynamic, c:Dynamic, d:Dynamic, e:Dynamic, f:Dynamic, g:Dynamic):Dynamic
						{
							return callScriptClassStaticFunction(clsName, fieldName, [a, b, c, d, e, f, g]);
						}

					case 8:
						return function(a:Dynamic, b:Dynamic, c:Dynamic, d:Dynamic, e:Dynamic, f:Dynamic, g:Dynamic, h:Dynamic):Dynamic
						{
							return callScriptClassStaticFunction(clsName, fieldName, [a, b, c, d, e, f, g, h]);
						}

					case _:
						@:privateAccess error(ECustom("only 8 params allowed in script class functions (.bind limitation)"));
					#end
				}
